...
import { $ as Briefcase, B as CreditCard, K as CircleAlert, L as DoorOpen, N as KeyRound, Q as Building2, a as UserCog, d as ShieldCheck, i as Users, n as Wrench, nt as BadgeCheck, r as Wallet, s as TrendingUp } from "../_libs/lucide-react.mjs";
import { t as Badge } from "./badge-D1Dupn2y.mjs";
import { t as Skeleton } from "./skeleton-D9W9wFsj.mjs";
import { a as SelectValue, i as SelectTrigger, n as SelectContent, r as SelectItem, t as Select } from "./select-Dg1urBTx.mjs";
import { n as companyTypeLabel, r as money, t as COMPANY_TYPES } from "./platform-Df7WJh8D.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/dashboard-BDglKdxS.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
function Dashboard() {
	const { access, can } = useAuth();
	return (access?.profile?.is_super_admin ?? false) && !access?.company ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(PlatformDashboard, {}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CompanyDashboard, {});
}
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [metrics.map((m) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Card, {
			...
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CardContent, {
				...
				children: "Your role has no dashboard metrics assigned yet. Ask your company administrator to grant permissions."
			})
		})]
	});
}
				...
				children: [
					...
					data?.recent.map((c) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							...
							children: [can("companies.suspend") ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Select, {
								value: c.company_type,
								onValueChange: (val) => updateCompany.mutate({
									id: c.id,
									patch: { company_type: val }
								}),
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			if (isClient && ownerId) {
				...
				unitsQuery = supabase.from("units").select("id, properties!inner(owner_id, branch_id)", head).eq("company_id", companyId).eq("properties.owner_id", ownerId);
				occQuery = supabase.from("units").select("id, properties!inner(owner_id, branch_id)", head).eq("company_id", companyId).eq("status", "occupied").eq("properties.owner_id", ownerId);
				uRowsQuery = supabase.from("units").select("rent, status, properties!inner(owner_id, branch_id)").eq("company_id", companyId).eq("properties.owner_id", ownerId);
			} else {
				unitsQuery = supabase.from("units").select("id, properties!inner(branch_id)", head).eq("company_id", companyId);
				occQuery = supabase.from("units").select("id, properties!inner(branch_id)", head).eq("company_id", companyId).eq("status", "occupied");
				uRowsQuery = supabase.from("units").select("rent, status, properties!inner(branch_id)").eq("company_id", companyId);
			}
			if (selectedBranch !== "all") {
				propQuery = propQuery.eq("branch_id", selectedBranch);
				unitsQuery = unitsQuery.eq("properties.branch_id", selectedBranch);
				occQuery = occQuery.eq("properties.branch_id", selectedBranch);
				uRowsQuery = uRowsQuery.eq("properties.branch_id", selectedBranch);
			}
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				}), branches && branches.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Select, {
