...
import { a as adminDeleteUser, f as sendEmailFn, o as adminResetTemporaryPassword, r as adminCreateOfficer, s as companyCreateEmployee } from "./platform.functions-BWY0p-Ku.mjs";
import { n as toast } from "../_libs/sonner.mjs";
import { V as Copy, W as CircleCheck, a as UserCog, c as Trash2, v as Plus } from "../_libs/lucide-react.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/employees-B1dHcvKD.js
...
function EmployeesPage() {
	...
	const { data: roles } = useQuery({
		...
		queryFn: async () => {
			...
			else query = query.is("company_id", null).in("slug", ["platform_verification_officer", "platform_support_officer"]);
	...
	const { data: employees, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			return profiles.filter((p) => p.position !== "Landlord" && p.position !== "Tenant" && p.is_super_admin !== true).map((p) => ({
				...p,
	...
	const createEmployee = useMutation({
		mutationFn: (input) => {
			if (!companyId && isSuper && input.roleSlug) return createOfficerFn({ data: {
				email: input.email,
