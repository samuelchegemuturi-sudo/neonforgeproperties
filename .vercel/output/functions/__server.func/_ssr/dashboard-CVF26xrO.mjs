...
//#region node_modules/.nitro/vite/services/ssr/assets/dashboard-CVF26xrO.js
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: async () => {
			...
			if (isClient && ownerId) {
				...
				unitsQuery = supabase.from("units").select("id, properties!inner(owner_id)", head).eq("company_id", companyId).eq("properties.owner_id", ownerId);
				occQuery = supabase.from("units").select("id, properties!inner(owner_id)", head).eq("company_id", companyId).eq("status", "occupied").eq("properties.owner_id", ownerId);
				uRowsQuery = supabase.from("units").select("rent, status, properties!inner(owner_id)").eq("company_id", companyId).eq("properties.owner_id", ownerId);
			}
