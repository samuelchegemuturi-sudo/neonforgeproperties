...
//#region node_modules/.nitro/vite/services/ssr/assets/platform.functions-DpVRRJQa.js
...
var adminCreateCompany = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => {
	...
	const positionName = data.company_type === "airbnb_host" ? "Host" : data.company_type === "property_management_agency" ? "Agency Admin" : data.company_type === "real_estate_company" ? "Broker" : data.company_type === "developer" ? "Developer" : data.company_type === "sacco" ? "SACCO Admin" : "Landlord";
	...
	if (landlordRole) {
		...
		await supabaseAdmin.from("roles").update({ name: positionName }).eq("id", landlordRole);
	}
...
var requestPasswordReset_createServerFn_handler = createServerRpc({
	id: "8103abbaf2058c695856cb672ded681604cf450dfff39937825fb48825202412",
	name: "requestPasswordReset",
	filename: "src/lib/platform.functions.ts"
}, (opts) => requestPasswordReset.__executeServer(opts));
var requestPasswordReset = createServerFn({ method: "POST" }).validator((input) => {
	if (!input.email?.trim()) throw new Error("Email is required");
	return input;
}).handler(requestPasswordReset_createServerFn_handler, async ({ data }) => {
	const { supabaseAdmin } = await import("./client.server-KzwUIAkW.mjs");
	...
	if (linkData?.properties?.action_link) {
		...
		if (brevoApiKey && brevoApiKey !== "\"\"") try {
...
var adminResetPassword = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => {
	if (!input.email?.trim()) throw new Error("Email is required");
	return input;
	...
	if (linkData?.properties?.action_link) {
		const { data: settings } = await supabaseAdmin.from("platform_settings").select("value").eq("key", "brevo_api_key").single();
		if (settings?.value && settings.value !== "\"\"") await fetch("https://api.brevo.com/v3/smtp/email", {
			method: "POST",
			headers: {
				"accept": "application/json",
				"api-key": String(settings.value),
				"content-type": "application/json"
			},
			body: JSON.stringify({
				sender: {
...
var adminCreateOfficer = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => {
	if (!input.email?.trim()) throw new Error("Email is required");
...
var companyCreateEmployee = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => {
	if (!input.email?.trim()) throw new Error("Email is required");
...
var adminResetTemporaryPassword = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => {
...
var sendEmailFn = createServerFn({ method: "POST" }).validator((input) => {
...
var adminDeleteUser = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => input).handler(adminDeleteUser_createServerFn_handler, async ({ data: { targetUserId }, context }) => {
	const { supabase, userId } = context;
...
var adminDeleteCompany = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => input).handler(adminDeleteCompany_createServerFn_handler, async ({ data: { targetCompanyId }, context }) => {
	const { supabase, userId } = context;
...
var registerCompanyFn = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => {
	...
	if (companyError) throw new Error(companyError.message);
	const positionName = data.company_type === "bnb_host" ? "Host" : data.company_type === "property_management_agency" ? "Agency Admin" : "Landlord";
	await supabaseAdmin.from("profiles").update({
		company_id: company.id,
		position: positionName
	}).eq("id", userId);
...
var activateTrialSubscriptionFn = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => input).handler(activateTrialSubscriptionFn_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
	const { supabaseAdmin } = await import("./client.server-KzwUIAkW.mjs");
...
var renewSubscriptionFn = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => input).handler(renewSubscriptionFn_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
	const { supabaseAdmin } = await import("./client.server-KzwUIAkW.mjs");
	const { data: roleData } = await supabase.from("user_roles").select("id").eq("user_id", userId).eq("company_id", data.company_id).limit(1);
	if (!roleData || roleData.length === 0) throw new Error("Forbidden - Not a member of this company");
...
var registerUserFn = createServerFn({ method: "POST" }).validator((input) => input).handler(registerUserFn_createServerFn_handler, async ({ data }) => {
	const { supabaseAdmin } = await import("./client.server-KzwUIAkW.mjs");
	const { data: created, error: authError } = await supabaseAdmin.auth.admin.createUser({
		email: data.email.trim(),
