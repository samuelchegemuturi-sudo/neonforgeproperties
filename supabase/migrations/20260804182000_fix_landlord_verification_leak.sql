-- 1. Redefine seed_company_roles without the 'verification' prefix for Landlord
...
-- 2. Delete verification permissions from all existing company roles
DELETE FROM public.role_permissions
WHERE permission_key LIKE 'verification.%'
AND role_id IN (
  SELECT id FROM public.roles WHERE company_id IS NOT NULL
);
